...
from scipy.stats import median_abs_deviation, shapiro, levene, kruskal, mannwhitneyu, false_discovery_control, fisher_exact, chisquare, ttest_rel, ttest_ind, wilcoxon, bartlett, f_oneway , tukey_hsd, friedmanchisquare
...
import itertools
import warnings
from core20260218 import OrganoidProperties, unique_list
...
def conservative_stat_decision_tree(data: pd.DataFrame, variable: str, group_columns: List[str], variable_type: str, what: str, day_dependance: str=None, force_parametric=False):
    ...
    for group_name in tmp_unpaired:
        unpaired_groups_all.append([sum(elt, ()) for elt in group_name])
    tmp_paired = [list(itertools.product(paired_groups,[pairs])) for pairs in unpaired_groups]
    paired_groups_all = []
    for group_name in tmp_paired:
        paired_groups_all.append([sum(elt, ()) for elt in group_name])
    ...
    if what == 'difference':
        ...
        if variable_type == 'ordinal':
            ...
            stat_group = data_grouped.describe()
            stat_group["IQR"] = stat_group["75%"] - stat_group["25%"] #adding IQR to stats
            stat_group["skew"] = data_grouped.skew()[variable]
            #add the test on outlier
            ...
            #Parametric Test
            if all(stat_group["count"] >= 25) and all(abs(stat_group["skew"]) <= 2) or force_parametric == True: #add the condition on outliers and extrema
                ...
                if len(paired_column) > 0:
                    ...
                    for group in paired_groups_all:
                        ...
                        if any(elt not in data_grouped_as_index.groups.keys() for elt in group):
                            continue

                        stats["Dependant"][str(group)] = {"Global": None, "Comparisons": None}  
                        print(stats["Dependant"])                      
                        ...
                        if len(group) <= 2:    
                            ...
                            stats["Dependant"][str(group)]["Comparisons"] = [{"Ngroup": 2, "Test": "Paired T-test", "Comparison": group, "P-value": pval}]     
                ...
                if len(unpaired_column) > 0 and len(unpaired_groups) > 1:
                    ...
                    for group in unpaired_groups_all:
                        ...
                        stats["Independant"][str(group)] = {"Global": None, "Comparisons": None}
            ...
            #Non-parametric Test
                ...
                #Unpaired Test    
                if len(unpaired_column) > 0:  

                    #Only two groups ===> MANN-WHITNEY
                    if len(stat_group.groupby(by=unpaired_column)) <= 2:  
                        pass
...
area = [9.5, 9.75, 10, 10.25, 10.5]

data = pd.DataFrame({
    "Hive_number": [0, 1, 2, 3, 4]*9,
    "Substrate": ["S1"]*5 + ["S2"]*5 + ["S3"]*5 + ["S1"]*5 + ["S2"]*5 + ["S3"]*5 +  ["S1"]*5 + ["S2"]*5 + ["S3"]*5,
    "Day": ["D1"]*15 + ["D4"]*15 + ["D8"]*15,
    ...
    "Condition": ["c1"]*5 + ["c2"]*5 + ["c1"]*5 + ["c1"]*5 + ["c2"]*5  + ["c1"]*5 + ["c1"]*5 + ["c2"]*5  + ["c1"]*5
})
print(data)

stats = conservative_stat_decision_tree(data, "area_filled", ["Day", "Substrate", "Condition"], 'continuous', 'difference', day_dependance='match', force_parametric=False)
print(type(stats))
pretty_print(stats)

...
#####
# TO DO:
#    -Manual discard: set rescale ratio in better way than as variable in methods.
#    -logs
#    -plots:
#       -info: exp, susbtrate name & condition, days
#       -boxplot of variable X:
#           -same condition in same box put different colors/markers for violin plot with autocomputation of marker/color distinction
#           -possibility to ungroup similar conditions
#           -displays: q1, q3, mean, median, outliers
#           -statistical tests of grouped data and ungrouped data
#           ------------------------------------------------------NEED TO BE DEFINED
#       -X,Y plot:
#           -mean +/- std or median +/- std. Grouped by default for same condition
#           -possibility to ungroup similar conditions
#           -possibility to display:
#               -number of organoidper group
#               -p-values in between 2 consecutive groups
#               -fit each group with a custom function and display param and R² in legend
#
#
# 
#####
...
def display_montage_organoids_dynamic(hierarchy: dict, obj: List[Exp], days: List[str], image_type: ImageType, ext=None):
    ...
    filename = image_type.value + hive_tmp.filenames_original
    if ext is not None:
            ...
            for h in hierarchy_real[exp][s]:
                for d in day_names_real:
                    ...
                    if d in list(hierarchy_obj[exp][s].keys()) and h in hierarchy_obj[exp][s][d]:
                        ...
                        filename = image_type.value + hive.filenames_original
...
class Exp:
    ...
    def create_montage(self, image_type: ImageType, hierarchy: dict=None, display_hive_number='on'):
        
        substrates, days = self.read_hierarchy(hierarchy)
        for s in self.substrates_refs:
            if s.name in substrates:
                idx_s = substrates.index(s.name)
                s.create_montage(image_type, days=days[idx_s], display_hive_number=display_hive_number)
    
    def crop(self, save_montage: bool=True, hierarchy: dict=None):
        """
        Crop all original pictures found and save the montage for each substrate and each days in the Experiment Folder.
        
        Parameters
            save_montage (bool): True by default.
                If none, do not create and do not save the associated montages.
            hierarchy (dict): None by default.
                If hierarchy is given, will crop the pictures only on requested substrates and days.
                The dictionnary should be structured as:
                hierarchy = {
                    "SubstrateA": ["D0", ..., "DN"],
                    "SubstrateB": ["D0", ..., "DN"]
                }
    ...
    def get_organoid_mask(self, hierarchy: dict=None):

        substrates, days = self.read_hierarchy(hierarchy)
        for s in self.substrates_refs:
            if s.name in substrates:
                idx_s = substrates.index(s.name)                
                s.generate_organoid_mask(display='off', verbose='off', days=days[idx_s]) #Organoid organoid
...
class Substrate:
    ...
    def create_montage(self, image_type: ImageType, days: List[str]=None, display_hive_number='on'):
        
        for d in self.days_refs:
            if days is not None:
                if d.name in days:
                    d.create_montage(image_type, remove=[], display_hive_number=display_hive_number)
            else:
                d.create_montage(image_type, remove=[], display_hive_number=display_hive_number)
    ...
    def generate_organoid_mask(self, display='off', verbose='off', manual_discard='off', days: List[str]=None):
...
class Day:
    ...
    def __init__(self, name, parent, current_param):
        ...
        self.filenames_core_mask = self.get_filenames(ImageType.CORE_MASK.value)
        self.filenames_shell_mask = self.get_filenames(ImageType.SHELL_MASK.value)
        ...
        self.hive_mask_rs = hexagon_outline_ndarray(dim_rs, center_rs, side_pxl_rs, thickness=5, return_type='hexagon')
    ...
    def find_rotation_angle(self, scale_ratio):
        ...
        for i, hive in enumerate(self.hives_refs):
            ...
            # img = hive.open_image(img_type=ImageType.ORIGINAL, rgb=True) #Open image
            # img_rs = resize(rgb2gray(img[i]), (round(img[i].shape[0]/scale_ratio), round(img[i].shape[1]/scale_ratio))) #Rescale image
            best_angles.append(hive.find_hive_rotation_angle(self.hive_mask_rs, int(self.current_param["Angle"]), 5, 0.5, scale_ratio)) #Compute & store best angle
    ...
    def create_montage(self, image_type: ImageType, remove: List[int]=[], display_hive_number='on', scale_ratio: int=6):
        # Substrate is definied as follow:
        # 1st column: 3 hives
        # 2nd column: 4 hives
        # 3rd column: 5 hives
        ...
        for hive, filename in zip(self.hives_refs, filenames):
            img[i] = hive.open_image(image_type=image_type, rgb=True)
            i+=1
        ...
        if display_hive_number =='on':
            ...
            for i, hive_number in enumerate(idx_flat):
                draw.text((coord[i][0][0]+25, coord[i][1][0]+50), str(hive_number), font=font, fill=(255,255,255))
            #montage_pil.show()
    ...
    def crop_hives(self):
        ...
        best_angle = self.find_rotation_angle(scale_ratio)

        #Crop & rotate image
        for i,hive in enumerate(self.hives_refs):
            print(f"{self.parent.name}_{self.name}, Cropping pictures... {i}/{n_hives}", end="\r")
            hive.crop_hive(scale_ratio, best_angle, save=True)
    ...
    def threshold_hives(self, display='off', verbose='off'):
        ...
        #         warnings.warn(f"No Optimized Crop images found for {self.parent.name}-{self.name}")
        #         return

        #     opticrop_img = []
        #     for hive in self.hives_refs:
        #         opticrop_img.append(hive.open_image(filename=hive.filenames_optimizedshadow, rgb=True))
        # else:
        ...
        for j,hive in enumerate(self.hives_refs):
            ...
            if display == 'on':
                ...
                current_ax = None
            thresholds.append(hive.custom_threshold_hive(fig1=fig, ax1=current_ax, title=f"#{hive.hive_number}", verbose=verbose))
    ...
    def generate_organoid_mask(self, display='off', verbose='off', manual_discard='off'):
        ...
        #     #Verify Optimized Crop images exist
        #     if len(self.filenames_optimized_shadow) == 0:
        #         warnings.warn("No Optimized Crop images found for {self.parent.name}-{self.name}")
        #         return
        ...
        for j,hive in enumerate(self.hives_refs):
            ...
            if display == 'on':
                ...
                current_ax_idx = None
            hive.generate_organoid_mask(fig=fig, ax=ax, ax_idx=current_ax_idx, verbose=verbose)
    ...
    def get_save_organoid_data(self, properties=OrganoidProperties.rg_properties.value, additionnal_props=OrganoidProperties.additionnal_props.value, metadata=OrganoidProperties.metadata.value, ResultFile="Results.csv"):
        ...
        for i,hive in enumerate(self.hives_refs):
            ...
            data_hive, organoid_count = hive.get_organoid_data(organoid_count, properties=properties, additionnal_props=additionnal_props, metadata=metadata)
            data.append(data_hive)
        print(f"{self.parent.name}_{self.name}, Getting organoid data done.    ", end="\r")
        print()

        #Save organoid data
        headers = properties + additionnal_props + metadata
    ...
    def manual_discard(self, overlay=None, newResultFile="UpdatedResults.csv", ResultFile="Results.csv", scale_ratio: int=6):
        ...
        overlay_path = self.path + path_delimiter +  "Montage_" + ImageType.OVERLAY.value + self.parent.parent.name + "_" + self.parent.name + "_" + self.name + ".jpg"
        print(os.path.isfile(overlay_path))
        ...
        for i in range(len(data)):
            ...
            coords.append({"Index": data[i]["Index"], \
                            "Hive number": data[i]["Hive number"], \
                            "Hive coord": coord_hives[idx_flatten.index(int(data[i]["Hive number"]))],
                            "bbox": bbox 
                            })
...
class Hive:
    ...
    def __init__(self, hive_number, parent, filenames_original):

        self.hive_number = hive_number
        self.parent = parent
        self.filenames_original = filenames_original
        self.name = self.parent.parent.parent.name + "_" + self.parent.parent.name + "_" + self.parent.name + "_" + str(self.hive_number) + ": " + self.filenames_original
        self.results = []
    ...
    def open_image(self, image_type, rgb=False, filename=None):
    ...
    def find_hive_rotation_angle(self, hive_mask, angle_user, angle_range, angle_step, scale_ratio):
        ...
        #Open image
        img = self.open_image(image_type=ImageType.ORIGINAL)
    ...
    def crop_hive(self, scale_ratio, best_angle, save=True):
        """Save (if save is True) cropped image and update crop coordinates self.crop_coord = (ymin, ymax, xmin, xmax)"""

        #Open and rescale image
        img = self.open_image(image_type=ImageType.ORIGINAL, rgb=True) #open image
        img_type = img.dtype

        (y_c, x_c) = self.get_hive_barycenter(best_angle, self.parent.hive_mask_rs, scale_ratio, image_type=ImageType.ORIGINAL) #compute hive barycenter
        ...
        if save == True:
            out_path = self.parent.path + path_delimiter + \
                ImageType.CROP.value + self.filenames_original
            if '.jpg' in out_path:
    ...
    def optimized_crop_hive(self, hives_mask_side, k_step,  display='off'):

        #Open image in grayscale
        img = self.open_image(image_type=ImageType.CROP, rgb=True)
        ...
        img[hive_mask == 0] = 0
        out_path = self.parent.path + path_delimiter + "OptimizedShadow_" + self.filenames_original
        if '.jpg' in out_path:
            out_path = out_path.replace('.jpg', '.tif')
        elif '.jpeg' in out_path:
            out_path = out_path.replace('.jpg', '.tif')
    ...
    def custom_threshold_hive(self, fig1=None, ax1=None, title=None, verbose='off'):
        ...
        #Open image in grayscale
        img = self.open_image(image_type=ImageType.OPTIMIZED_SHADOW, rgb=True)
        img_gray = rgb2gray(img)
    ...
    def generate_organoid_mask(self, fig=None, ax=None, ax_idx=None, verbose='off'):
        ...
        #Apply threshold
        img_thresh = (img_blur > self.threshold[0]) & (img_blur <= self.threshold[1])
        ...
        #Save mask
        out_path = self.parent.path + path_delimiter +  ImageType.MASK.value + self.filenames_original
        if '.jpg' in out_path:
            out_path = out_path.replace('.jpg', '.tif')
        elif '.jpeg' in out_path:
            out_path = out_path.replace('.jpg', '.tif')
        ...
        overlay[:,:,1] = img[:,:,1] #G
        overlay[:,:,2] = img[:,:,2] #B
        out_path = self.parent.path + path_delimiter +  ImageType.OVERLAY.value + self.filenames_original
        if '.jpg' in out_path:
            out_path = out_path.replace('.jpg', '.tif')
        elif '.jpeg' in out_path:
            out_path = out_path.replace('.jpg', '.tif')
    ...
    def get_organoid_data(self, organoid_count, properties=OrganoidProperties.rg_properties, additionnal_props=OrganoidProperties.additionnal_props, metadata=OrganoidProperties.metadata.value):
        """
        ...
        **Anavlysis Version**: str
        ...
        #Open mask
        mask_img = self.open_image(rgb=True, image_type=ImageType.MASK)
        
        #Open original image
        img = self.open_image(rgb=True, image_type=ImageType.OPTIMIZED_SHADOW)

        #Extract info using region props
        labels = label(mask_img)
        pxl2um = 1/self.parent.um2pxl
        regions = regionprops(labels, img, extra_properties=(intensity_median,intensity_mad), spacing=(pxl2um, pxl2um))
        data = []
        for rg in regions:
            ...
            data[-1].update({"Index": organoid_count, "Day": self.parent.name, "Substrate": self.parent.parent.name,
                             "Image Name": self.filenames_original, "Hive number": image_number_from_filename(self.filenames_original, format='int'),
    ...
    def get_hive_barycenter(self, best_angle, hive_mask, scale_ratio, image_type: ImageType):

        #Open image
        img = self.open_image(image_type=image_type)
        img = resize(img, (round(img.shape[0]/scale_ratio), round(img.shape[1]/scale_ratio)))
        # if image is None:
        #     img = self.open_image(image_type=image_type)
        #     img = resize(img, (round(img.shape[0]/scale_ratio), round(img.shape[1]/scale_ratio)))
...
class ImageData:
    ...
    def __init__(self,channel_names: ChannelNames, filename: str):
        
        self.image_data = {channel_names.name: {(0,0,0): filename}} #(channel_number, z_stack, timepoint)
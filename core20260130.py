...
class Substrate:
    ...
    def threshold_hives(self, display='off', verbose='off', days: List[str]=None):
        ...
        for d in self.days_refs:
            if days is not None:
                if d.name in days:
                    d.threshold_hives(opticrop_image=None, display=display, verbose=verbose)
            else:
                d.threshold_hives(opticrop_image=None, display=display, verbose=verbose)
    ...
    def generate_organoid_mask(self, display='off', verbose='off', manual_discard='off', days: List[str]=None):
        ...
        for d in self.days_refs:
            if days is not None:
                if d.name in days:
                    d.generate_organoid_mask(opticrop_image=None, display=display, verbose=verbose)
            else:
                d.generate_organoid_mask(opticrop_image=None, display=display, verbose=verbose)
...
class Day:
    ...
    def optimized_crop_hives(self, n_slice: int=15, k_step: int=10):
        ...
        self.filenames_optimized_shadow = self.get_filenames(ImageType.OPTIMIZED_SHADOW.value)
        for hive in self.hives_refs:
            print(hive.filenames_optimizedshadow)
    ...
    def generate_organoid_mask(self, opticrop_image=None, display='off', verbose='off', manual_discard='off'):
    ...
    def get_save_organoid_data(self, opticrop_image=None, mask_image=None, properties=OrganoidProperties.rg_properties.value, additionnal_props=OrganoidProperties.additionnal_props.value, metadata=OrganoidProperties.metadata.value, ResultFile="Results.csv"):

        #Open mask pictures
        if mask_image is None:

            #Verify Mask images exist
            if len(self.filenames_mask) == 0:
                warnings.warn(f"No Mask images found for {self.parent.name}-{self.name}")
                return
            
            mask_img = []
            for hive in self.hives_refs:
                mask_img.append(hive.open_image(filename=hive.filenames_mask, rgb=True))
        else:
            mask_img = mask_image
        ...
        if opticrop_image is None:
            ...
            #Verify Mask images exist
            if len(self.filenames_optimized_shadow) == 0:
                warnings.warn(f"No Optimized Shadow images found for {self.parent.name}-{self.name}")
                return

            opticrop_img = []
            for hive in self.hives_refs:
                opticrop_img.append(hive.open_image(filename=hive.filenames_optimizedshadow, rgb=False))
        else:
            opticrop_img = opticrop_image
        ...
        for i,hive in enumerate(self.hives_refs):
            ...
            data_hive, organoid_count = hive.get_organoid_data(organoid_count, image=opticrop_img[i], mask_image=mask_img[i], properties=properties, additionnal_props=additionnal_props, metadata=metadata)
...
class Hive:
    ...
    def __init__(self, hive_number, parent, filenames_original):
        ...
        self.filenames_optimizedshadow = self.update_filename(ImageType.OPTIMIZED_SHADOW)
    ...
    def open_image(self, rgb=False, img_type=None, filename=None):
        ...
        if filename is None:
            file_name = self.filenames_original
        else:
            file_name = filename
    ...
    def optimized_crop_hive(self, hives_mask_side, k_step, crop_img=None, display='off'):

        #Open image in grayscale
        if crop_img is None:
            # filename = ImageType.CROP.value + self.filenames_original
            # if '.jpg' in filename:
            #     filename = filename.replace('.jpg', '.tif')
            # elif '.jpeg' in filename:
            #     filename = filename.replace('.jpg', '.tif')
            img = self.open_image(rgb=True, filename=self.filenames_crop)
        else: 
            img = crop_img
    ...
    def custom_threshold_hive(self, opticrop_img=None, fig1=None, ax1=None, title=None, verbose='off'):
        ...
        if opticrop_img is None:
            # filename = ImageType.OPTIMIZED_SHADOW.value + self.filenames_original
            # if '.jpg' in filename:
            #     filename = filename.replace('.jpg', '.tif')
            # elif '.jpeg' in filename:
            #     filename = filename.replace('.jpg', '.tif')
            img = self.open_image(rgb=True, filename=self.filenames_optimizedshadow)
        else: 
            img = opticrop_img
        img_gray = rgb2gray(img)
    ...
    def get_organoid_data(self, organoid_count, image=None, mask_image=None, properties=OrganoidProperties.rg_properties, additionnal_props=OrganoidProperties.additionnal_props, metadata=OrganoidProperties.metadata.value):
        """
        ...
        **Anavlysis Version**: str
        ...
        #Open mask
        if mask_image is None:
            mask_img = self.open_image(filename=self.filenames_mask)
        else: 
            mask_img = mask_image
        
        #Open original image
        if image is None:
            img = self.open_image(filename=self.filenames_optimizedshadow)
        else: 
            img = image
        ...
        # plt.imshow(labels)
        # plt.show()
        regions = regionprops(labels, img, extra_properties=(intensity_median,intensity_mad), spacing=(0.9,0.9))
        data = []
        for rg in regions:
